...
class SVM:
    ...
    def build_models(self):
        ...
        model1 = Pipeline([
            ...
            ('regressor', SVR(kernel='linear'))  # Support Vector Regression with a linear kernel
        ])

        model2 = Pipeline([
            ('scaler', StandardScaler()),  # Standardize features
            ('classifier', SVC(kernel='rbf', C=1.0))  # Support Vector Classification with a radial basis function kernel
        ])

        model3 = Pipeline([('scaler', StandardScaler()),
                           ('classifier', SVC(kernel='rbf',  C=7.8, gamma=4.55))
						  ])
